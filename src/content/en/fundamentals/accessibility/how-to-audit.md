...
users see or experience the world as you do. Because humans are diverse, and
have a range of abilities, there may be some users who require additional help
to access the content on a page. It’s important to consider the perspective of
these users when designing and developing your site, and to routinely audit your
site to test that it is flexible enough to cater to the needs of all users. This
post covers the why, and how, of doing an accessibility audit to ensure that
...
different user perspectives. You may find yourself asking questions like "Can we
make the keyboard navigation more efficient? Can we improve the contrast of this
body copy? Should we move these buttons and labels closer together?" Invariably
this leads to a product that is easier for all users to interact with.
...
For users who either cannot, or choose not to use a mouse, keyboard navigation
is their primary means of reaching everything on screen. This audience includes
users with a motor impairment, such as Repetitive Stress Injury (RSI) or
...
- No controls have a `tabindex` > 0. These controls will jump ahead of
everything else in the tab order, regardless of their position in the DOM. This
is can be confusing for screen reader users as they tend to navigate the DOM in
a linear fashion.

- Custom interactive controls are focusable. If you use JavaScript to turn a
`<div>` into a fancy dropdown, it will not automatically be inserted into the
tab order. To make a custom control focusable, give it a `tabindex=”0”`.

- Non-interactive content (e.g. headings) should not be focusable. Sometimes
developers will add a `tabindex` to headings because they think they’re
important. This is actually an anti-pattern because it makes keyboard users who
can see the screen less efficient. For screen reader users, the screen reader
will already announce these headings, so there’s no need to make them focusable.
...
where keyboard focus may get stuck. The only time focus should be trapped is in
specific situations, like when displaying a modal, when you wouldn’t want the
...
Many sites have offscreen content which is present in the DOM but not visible.
...
content. To signify that an image should be skipped by a screen reader you can
set the value of the `alt` attribute to an empty string, e.g. `alt=””`.
...
in the page, try to move it earlier in the DOM.
...
need a `role=”checkbox”` and `aria-checked=”true|false”` to properly convey its
...
easy to pick up. In general, most developer can get by with just a few simple
key commands.
...
element, it has no effect on the _behavior_ of that element. So while you can
...
shortcuts and pass through all key presses to the page. This means the keys
...
levels are appropriate and see which landmarks are in use. To learn more check
out these instructional videos on the basics of
...
- Does the page pass all the tests from either the aXe or WAVE browser
extensions? These extensions are a useful addition to any manual test process as
they can quickly pick up on subtle mistakes like failing contrast ratios and
missing ARIA attributes. If you prefer to do things from the command line,
[axe-cli](https://github.com/dequelabs/axe-cli) provides the same functionality
as the browser extension, but can be easily run from your terminal.

- To avoid regressions, especially in a continuous integration environment,
incorporate a library like axe-core into your automated test suite. axe-core is
the same engine that powers the aXe chrome extension, but in an easy to run
command line utility.

- If you’re using a framework or library, does it provide it’s own accessibility
tools? Some examples include protractor-accessibility-plugin for Angular, and
a11ysuite for Polymer and Web Components. Take advantage of these tools whenever
...
- Web Accessibility by Google (free online course)
- Accessibility Fundamentals (documentation)
